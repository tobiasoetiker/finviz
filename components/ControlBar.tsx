            ...
            <div className="max-w-[1600px] mx-auto flex flex-col gap-4 md:gap-5">
                ...
                <div className="flex md:hidden items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Market Snapshot</span>
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl font-bold text-xs transition-all active:scale-95 border border-blue-100"
                    >
                        {isExpanded ? 'Close Filters' : 'Adjust Filters'}
                        <ChevronDown size={14} className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                    </button>
                </div>

                {/* Main Controls - Collapsible on Mobile */}
                <div className={`${isExpanded ? 'flex' : 'hidden md:flex'} flex-col gap-5`}>
